...
const PORT_ENV = Number(process.env.PRIVATEPROMPT_PORT);
...
try {
  ...
  console.error(`[privateprompt] cannot create data dir ${PROMPTS_DIR}: ${err.message}`);
  process.exit(1);
}
...
// One prompt file per repo/cwd so concurrent /privateprompt calls from
// different projects never clobber each other. No cwd -> shared scratch file.
function promptFileFor(cwd) {
  if (!cwd || typeof cwd !== "string" || !cwd.trim()) return LEGACY_PROMPT_FILE;
...
function historyFileFor(cwd) {
  if (!cwd || typeof cwd !== "string" || !cwd.trim()) return LEGACY_HISTORY_FILE;
  const hash = crypto.createHash("sha1").update(cwd).digest("hex").slice(0, 12);
...
function appendHistory(cwd, content) {
  ...
  try {
    ...
    lines = []; // corrupt/unreadable history — start fresh rather than fail the save
  }
  lines.push(JSON.stringify({ ts: Date.now(), content }));
  if (lines.length > HISTORY_LIMIT) lines = lines.slice(lines.length - HISTORY_LIMIT);
  try {
    fs.writeFileSync(file, lines.join("\n") + "\n", "utf8");
  } catch (err) {
    console.error(`[privateprompt] failed to append history: ${err.message}`); // non-fatal — save itself already succeeded
  }
}
...
function readHistory(cwd) {
  ...
  for (const line of lines) {
    try {
      ...
      if (parsed && typeof parsed.content === "string" && typeof parsed.ts === "number") entries.push(parsed);
    } catch {
      // one bad line shouldn't sink the whole history
    }
  }
...
function enhanceWithCodex(instruction, model, cwd, callback) {
  let tempDir;
  try {
    tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "privateprompt-"));
  } catch (err) {
    callback(err);
    return;
  }
...
const server = http.createServer(async (req, res) => {
  ...
  try {
    ...
    if (req.method === "POST" && url.pathname === "/save") {
      ...
      if (content.trim()) appendHistory(data.cwd, content); // don't clutter history with blanks
    ...
    console.error("[privateprompt] unhandled error:", err);
...
server.on("error", (err) => {
  if (err.code === "EADDRINUSE") {
    ...
    console.error(`[privateprompt] port ${PORT} already in use — assuming another instance is running`);
    process.exit(0);
  }
  console.error("[privateprompt] server error:", err);
  process.exit(1);
});

server.listen(PORT, "127.0.0.1", () => {
  console.error(`[privateprompt] listening on http://127.0.0.1:${PORT}`);
});
